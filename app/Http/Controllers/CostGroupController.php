...
class CostGroupController extends Controller
{
    ...
    public function dataTableJson(Request $request)
    {
        ...
        $costgroups = $allData->slice($skip)->take($limit);
        ...
        $response = [
            ...
            'recordsTotal' => CostGroup::count(),
            'recordsFiltered' => $total_data_after_search,
            'data' => $costgroups->values()->toArray(),
        ];
    ...
    {
        try {
            $User = CostGroup::where('KODE', $KODE)->first();
            if (!$User) {
    ...
    {
        try {
            //get all Cost Group order by KODE
            $costGroup = CostGroup::orderBy('KODE')->get();
            return ApiResponse::json(true, 'Data retrieved successfully',  $costGroup);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to retrieve data', null, 500);
        }
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'KODE' => 'required|unique:cost_groups,KODE',
            'NAMA' => 'required|unique:cost_groups,NAMA',
        ], [
        ...
        $costGroup = CostGroup::where(DB::raw('LOWER("KODE")'), strtolower($request->KODE))->first();
        if ($costGroup) {
            return ApiResponse::json(false, ['KODE' => ['The KODE Cost Group has already been taken.']], null, 422);
        }

        // Check uniqueness for NAMA case insensitive
        $costGroup = CostGroup::where(DB::raw('LOWER("NAMA")'), strtolower($request->NAMA))->first();
        if ($costGroup) {
        ...
        try {
            ...
            $new_truck = new CostGroup();
            $new_truck->KODE = $validatedData['KODE'];
    ...
    {
        $validator = Validator::make($request->all(), [
            //KODE required unique except for current KODE
            'KODE' => 'required|unique:cost_groups,KODE,' . $KODE . ',KODE',
            'NAMA' => 'required|unique:cost_groups,NAMA,' . $KODE . ',KODE',
        ], [
            'NAMA.required' => 'The NAMA field is required.',
            'NAMA.unique' => 'The NAMA has already been taken.',
            'KODE.unique' => 'The KODE has already been taken.',
        ...
        $costGroup = CostGroup::where(DB::raw('LOWER("KODE")'), strtolower($request->KODE))->where('KODE', '!=', $KODE)->first();
        if ($costGroup) {
        ...
        $costGroup = CostGroup::where(DB::raw('LOWER("NAMA")'), strtolower($request->NAMA))->where('KODE', '!=', $KODE)->first();
        if ($costGroup) {
            return ApiResponse::json(false, ['NAMA' => ['The NAMA has already been taken.']], null, 422);
        }
        ...
        try {
            $validatedData = $validator->validated();
            $costGroup = CostGroup::findOrFail($KODE);
            if ($costGroup->hasRelatedRecords()) {
                return ApiResponse::json(false, 'Cannot update cost group because it has related records', null, 400);
            }
            $costGroup->KODE = $validatedData['KODE'];
            $costGroup->NAMA = $validatedData['NAMA'];
            $costGroup->save();
            if (!$costGroup) {
            ...
            return ApiResponse::json(true, 'Cost Group successfully updated', $resp_containertype);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to update Cost Group', null, 500);
        }
    }
    ...
    public function destroy($KODE)
    {
        try {
            $costGroup = CostGroup::findOrFail($KODE);
            $costGroup->delete();
            return ApiResponse::json(true, 'Cost Group successfully deleted', ['KODE' => $KODE]);
        } catch (\Exception $e) {
            return ApiResponse::json(false, $e->getMessage(), null, 500);
        }
    }
    ...
    public function trash()
    {
        ...
        $costGroups = CostGroup::onlyTrashed()->get();

        return ApiResponse::json(true, 'Trash bin fetched',  $costGroups);
    }

    public function restore($id)
    {
        $restored = CostGroup::onlyTrashed()->findOrFail($id);
        $restored->restore();
    ...
    {
        $post = CostGroup::onlyTrashed()->findOrFail($id);
    ...
    {
        // $tableName = 'cost_groups'; // Replace with your table name
        ...
        $maxKODE = CostGroup::withTrashed()->max('KODE');
        //get next KODE
        $nextKODE = $maxKODE + 1;
