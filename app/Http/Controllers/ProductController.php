<?php

namespace App\Http\Controllers;

use App\Helpers\ApiResponse;
use App\Models\Category;
use App\Models\Formula;
use App\Models\Product;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Validator;

class ProductController extends Controller
{
    public function getProductDropdown()
    {
        try {
            $products = Product::orderBy('KODE')->get();

            foreach ($products as $product) {
                $category = Category::where('KODE', $product->KODE_KATEGORI)->first();
                $formula = Formula::where('KODE', $product->KODE_RUMUS)->first();

                $product->NAMA_KATEGORI = $category->NAMA_KATEGORI;
                $product->NAMA_RUMUS = $formula->NAMA_RUMUS;
            }

            return response()->json(['data' => $products]);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Failed to retrieve data'], 500);
        }
    }

    public function dataTableJson(Request $request)
    {
        // try {
        $order = $request->input('order')[0];
        $columnIndex = (int) $order['column'];
        $sortDirection = $order['dir'];

        $columns = $request->input('columns');
        $columnToSort = $columns[$columnIndex]['data'];

        $query = Product::query();

        $hasSearch = $request->has('search') && !empty($request->input('search')['value']);
        $dateColumns = ["TGL_REG"]; // Add your date column names here
        // Apply search filter if applicable
        if ($hasSearch) {
            $searchValue = strtoupper($request->input('search')['value']);
            $attributes = Product::first()->getAttributes(); // Get all attribute names
            $query->where(function ($q) use ($searchValue, $attributes, $dateColumns) {
                foreach ($attributes as $attribute => $value) {

                    if (in_array($attribute, $dateColumns)) {
                        // Modify the attribute value if it's a date column
                        $q->orWhere(DB::raw("TO_CHAR(\"$attribute\", 'dd-mm-yyyy')"), 'LIKE', "%$searchValue%");
                    } else {
                        $q->orWhere($attribute, 'LIKE', "%$searchValue%");
                    }
                }
            });

            $relatedColumns = [
                'category' => [
                    'table' => 'categories',
                    'column' => 'NAMA_KATEGORI',
                ],
                'formula' => [
                    'table' => 'formulas',
                    'column' => 'NAMA_RUMUS',
                ],

            ];

            foreach ($relatedColumns as $relation => $relatedColumn) {
                $query->orWhereHas($relation, function ($q) use ($searchValue, $relatedColumn) {
                    $q->where($relatedColumn['column'], 'LIKE', "%$searchValue%");
                });
            }
        }

        $query->orderByRaw('CAST(SUBSTRING("KODE" FROM \'\\.([0-9]+)$\') AS INTEGER) ASC, "KODE" ASC');

        $allData = $query->get();



        if (!empty($columnToSort)) {
            foreach ($allData as $product) {
                $product->NAMA_KATEGORI = Category::where('KODE', $product->KODE_KATEGORI)->first()->NAMA_KATEGORI;
                $product->NAMA_RUMUS = Formula::where('KODE', $product->KODE_RUMUS)->first()->NAMA_RUMUS;
            }
            $sortBoolean = $sortDirection === 'asc' ? false : true;
            //check if columnToSort is KODE
            if ($columnIndex === 1) {


                //include the sort boolean
                $allData = $allData->sortBy(function ($data) {
                    return (int) substr($data->KODE, strpos($data->KODE, '.') + 1);
                }, SORT_REGULAR, $sortBoolean);
            } else {
                $allData = $allData->sortBy($columnToSort, SORT_REGULAR, $sortBoolean);
            }
        }



        // Apply pagination and limit
        $limit = $request->get('length', 10);
        $total_data_after_search = $query->count();
        if ($limit == -1) {
            $limit = $total_data_after_search; // Use the total count after search filters
        }
        $page = $request->get('start', 0) / $limit + 1;
        $skip = ($page - 1) * $limit;

        // $customers = array_slice($allDataArray, $skip, $limit);
        $products = $allData->slice($skip)->take($limit);



        // ... rest of your code to enrich the data ...
        if (empty($columnToSort)) {
            foreach ($products as $product) {
                $product->NAMA_KATEGORI = Category::where('KODE', $product->KODE_KATEGORI)->first()->NAMA_KATEGORI;
                $product->NAMA_RUMUS = Formula::where('KODE', $product->KODE_RUMUS)->first()->NAMA_RUMUS;
            }
        }



        // Prepare the response data structure expected by the frontend

        $response = [
            'draw' => $request->input('draw'),
            'recordsTotal' => Product::count(),
            'recordsFiltered' => $total_data_after_search,
            'data' => $products->values()->toArray(),
        ];



        return ApiResponse::json(true, 'Data retrieved successfully', $response);
        // } catch (\Exception $e) {
        //     return ApiResponse::json(false, $e, null, 500);
        // }
    }

    public function dataTableAdvJson(Request $request)
    {
        //related to function
        $relationColumnsAndTo = [
            3 => ['category', 'NAMA_KATEGORI'],

            4 => ['formula', 'NAMA_RUMUS'],

        ];
        try {
            $order = $request->input('order')[0];
            $columnIndex = (int) $order['column'];
            $sortDirection = $order['dir'];

            $columns = $request->input('columns');
            $columnToSort = $columns[$columnIndex]['data'];

            $query = Product::query();

            //check for each column hasSearch columns[x][search][value]
            $columns = $request->input('columns');
            $hasSearch = false;
            foreach ($columns as $index => $column) {
                if (isset($column['search']['value']) && !empty($column['search']['value'])) {
                    $hasSearch = true;
                    break;
                }
            }


            $dateColumns = []; // Add your date column names here

            if ($hasSearch) {
                foreach ($columns as $index => $column) {
                    if (isset($column['search']['value']) && !empty($column['search']['value'])) {
                        if ($column['searchable'] === 'true') {
                            $searchValue = strtoupper($column['search']['value']);
                            // Remove parentheses from search value
                            $searchValue = str_replace(['(', ')'], '', $searchValue);

                            if (array_key_exists($index, $relationColumnsAndTo)) {
                                [$relation, $relatedColumn] = $relationColumnsAndTo[$index];

                                $query->whereHas($relation, function ($q) use ($searchValue, $relatedColumn) {
                                    $q->where($relatedColumn, 'LIKE', "%$searchValue%");
                                });
                            } else {
                                $columnName = $columns[$index]['data'];
                                if (in_array($columnName, $dateColumns)) {
                                    // Modify the attribute value if it's a date column
                                    $query->where(DB::raw("TO_CHAR(\"$columnName\", 'dd-mm-yyyy')"), 'LIKE', "%$searchValue%");
                                } else {
                                    $query->where($columnName, 'LIKE', "%$searchValue%");
                                }
                            }
                        }
                    }
                }
            }

            $query->orderByRaw('CAST(SUBSTRING("KODE" FROM \'\\.([0-9]+)$\') AS INTEGER) ASC, "KODE" ASC');

            $allData = $query->get();



            if (!empty($columnToSort)) {
                foreach ($allData as $product) {
                    $product->NAMA_KATEGORI = Category::where('KODE', $product->KODE_KATEGORI)->first()->NAMA_KATEGORI;
                    $product->NAMA_RUMUS = Formula::where('KODE', $product->KODE_RUMUS)->first()->NAMA_RUMUS;
                }
                $sortBoolean = $sortDirection === 'asc' ? false : true;
                //check if columnToSort is KODE
                if ($columnIndex === 1) {


                    //include the sort boolean
                    $allData = $allData->sortBy(function ($data) {
                        return (int) substr($data->KODE, strpos($data->KODE, '.') + 1);
                    }, SORT_REGULAR, $sortBoolean);
                } else {
                    $allData = $allData->sortBy($columnToSort, SORT_REGULAR, $sortBoolean);
                }
            }

            // Convert the collection of cities to an array

            // Apply pagination and limit
            $limit = $request->get('length', 10);
            $total_data_after_search = $query->count();
            if ($limit == -1) {
                $limit = $total_data_after_search; // Use the total count after search filters
            }
            $page = $request->get('start', 0) / $limit + 1;
            $skip = ($page - 1) * $limit;

            $products = $allData->slice($skip)->take($limit);

            if (empty($columnToSort)) {
                foreach ($products as $product) {
                    $product->NAMA_KATEGORI = Category::where('KODE', $product->KODE_KATEGORI)->first()->NAMA_KATEGORI;
                    $product->NAMA_RUMUS = Formula::where('KODE', $product->KODE_RUMUS)->first()->NAMA_RUMUS;
                }
            }


            // ... rest of your code to enrich the data ...




            $response = [
                'draw' => $request->input('draw'),
                'recordsTotal' => Product::count(),
                'recordsFiltered' => $total_data_after_search,
                'data' => $products->values()->toArray(),
            ];

            return ApiResponse::json(true, 'Data retrieved successfully', $response);
        } catch (\Exception $e) {
            return ApiResponse::json(false, $e, null, 500);
        }
    }
    public function findByKode($KODE)
    {
        try {
            $product = Product::where('KODE', $KODE)->first();
            //convert to array
            $product = $product->toArray();
            return ApiResponse::json(true, 'Category retrieved successfully',  $product);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to retrieve Category', null, 500);
        }
    }

    public function index()
    {
        try {
            //get all Category order by KODE
            $products = Product::orderBy('KODE')->get();
            foreach ($products as $product) {
                $product->NAMA_KATEGORI = Category::where('KODE', $product->KODE_KATEGORI)->first()->NAMA_KATEGORI;
                $product->NAMA_RUMUS = Formula::where('KODE', $product->KODE_RUMUS)->first()->NAMA_RUMUS;
            }

            return ApiResponse::json(true, 'Data retrieved successfully',  $products);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to retrieve data', null, 500);
        }
    }

    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            // unique case insensitive
            'NAMA_PRODUK' => 'required|unique:products,NAMA_PRODUK',
            'KODE_KATEGORI' => 'required',
            'KODE_RUMUS' => 'required',
        ], []);

        if ($validator->fails()) {
            $errors = $validator->errors();
            return ApiResponse::json(false, $errors, null, 422);
        }

        // Check uniqueness for NAMA_PRODUK_KATEGORI case insensitive
        $existingData = Product::where(DB::raw('LOWER("NAMA_PRODUK")'), strtolower($request->NAMA_PRODUK))->first();
        if ($existingData) {
            return ApiResponse::json(false, ['NAMA_PRODUK' => ['The NAMA_PRODUK has already been taken.']], null, 422);
        }



        try {
            $validatedData = $validator->validated();

            $new_Product = new Product();
            $new_Product->KODE = $this->getLocalNextId();
            $new_Product->NAMA_PRODUK = $validatedData['NAMA_PRODUK'];
            $new_Product->KODE_KATEGORI = $validatedData['KODE_KATEGORI'];
            $new_Product->KODE_RUMUS = $validatedData['KODE_RUMUS'];
            $new_Product->save();



            if (!$new_Product) {
                return ApiResponse::json(false, 'Failed to insert data', null, 500);
            }

            //include NAMA_KATEGORI in $new_Product
            $new_Product->NAMA_KATEGORI = Category::where('KODE', $new_Product->KODE_KATEGORI)->first()->NAMA_KATEGORI;
            $new_Product->NAMA_RUMUS = Formula::where('KODE', $new_Product->KODE_RUMUS)->first()->NAMA_RUMUS;


            return ApiResponse::json(true, "Data inserted successfully with KODE $new_Product->KODE", $new_Product, 201);
        } catch (\Exception $e) {
            return ApiResponse::json(false, $e->getMessage(), null, 500);
        }
    }
    public function update(Request $request, $KODE)
    {
        $validator = Validator::make($request->all(), [
            // unique case insensitive
            'NAMA_PRODUK' => 'required|unique:products,NAMA_PRODUK,' . $KODE . ',KODE',
            'KODE_KATEGORI' => 'required',
            'KODE_RUMUS' => 'required',
        ], []);

        if ($validator->fails()) {
            $errors = $validator->errors();
            return ApiResponse::json(false, $errors, null, 422);
        }

        // Check uniqueness for NAMA_PRODUK_KATEGORI case insensitive
        $existingData = Product::where(DB::raw('LOWER("NAMA_PRODUK")'), strtolower($request->NAMA_PRODUK))->where('KODE', '!=', $KODE)->first();;
        if ($existingData) {
            return ApiResponse::json(false, ['NAMA_PRODUK' => ['The NAMA_PRODUK has already been taken.']], null, 422);
        }



        try {
            $validatedData = $validator->validated();
            $product = Product::findOrFail($KODE);
            if ($product->hasRelatedRecords()) {
                return ApiResponse::json(false, 'Cannot update product because it has related records', null, 400);
            }
            $product->NAMA_PRODUK = $validatedData['NAMA_PRODUK'];
            $product->KODE_KATEGORI = $validatedData['KODE_KATEGORI'];
            $product->KODE_RUMUS = $validatedData['KODE_RUMUS'];
            $product->save();
            if (!$product) {
                return ApiResponse::json(false, 'Failed to update product', null, 500);
            }

            //include NAMA_KATEGORI in $product
            $product->NAMA_KATEGORI = Category::where('KODE', $product->KODE_KATEGORI)->first()->NAMA_KATEGORI;
            $product->NAMA_RUMUS = Formula::where('KODE', $product->KODE_RUMUS)->first()->NAMA_RUMUS;



            return ApiResponse::json(true, 'Category successfully updated', $product);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to update Category', null, 500);
        }
    }

    public function destroy($KODE)
    {
        try {
            $product = Product::findOrFail($KODE);
            $product->delete();
            return ApiResponse::json(true, 'Category successfully deleted', ['KODE' => $KODE]);
        } catch (\Exception $e) {
            return ApiResponse::json(false, $e->getMessage(), null, 500);
        }
    }

    // public function trash()
    // {

    //     $products = Product::onlyTrashed()->get();

    //     return ApiResponse::json(true, 'Trash bin fetched',  $products);
    // }


    public function trash()
    {
        $provinces = Product::onlyTrashed()->get();

        $resp_provinces = [];

        foreach ($provinces as $harbor) {

            $resp_province = [
                'KODE' => $harbor->KODE,
                'NAMA_PRODUK' => $harbor->NAMA_PRODUK,
                'NAMA_KATEGORI' => Category::where('KODE', $harbor->KODE_KATEGORI)->first()->NAMA_KATEGORI,
                'NAMA_RUMUS' => Formula::where('KODE', $harbor->KODE_RUMUS)->first()->NAMA_RUMUS,
            ];

            $resp_provinces[] = $resp_province;
        }

        return ApiResponse::json(true, 'Trash bin fetched', $resp_provinces);
    }


    public function restore($id)
    {
        $restored = Product::onlyTrashed()->findOrFail($id);
        $restored->restore();

        return ApiResponse::json(true, 'Restored', ['KODE' => $id]);
    }





    public function getNextId()
    {
        $nextId = $this->getLocalNextId();

        return ApiResponse::json(true, 'Next KODE retrieved successfully', $nextId);
    }

    public function getLocalNextId()
    {
        // Get the maximum COM.X ID from the database
        $maxId = Product::where('KODE', 'LIKE', 'PRD.%')
            ->withTrashed()
            ->orderByRaw('CAST(SUBSTRING("KODE" FROM \'\\.([0-9]+)$\') AS INTEGER) DESC, "KODE" DESC')
            ->value('KODE');

        if ($maxId) {
            // Extract the number from the maximum ID
            $maxNumber = (int) substr($maxId, strpos($maxId, '.') + 1);

            // Increment the number
            $nextNumber = $maxNumber + 1;
        } else {
            // If no existing IDs found, start with 1
            $nextNumber = 1;
        }

        // Create the next ID by concatenating 'COM.' with the incremented number
        $nextId = 'PRD.' . $nextNumber;

        return $nextId;
    }
}
