...
class CategoryController extends Controller
{
    ...
    public function dataTableJson(Request $request)
    {
        try {
            ...
            if ($hasSearch) {
                ...
                $attributes = Category::first()->getAttributes(); // Get all attribute names
            ...
            $categories = $allData->slice($skip)->take($limit);
    ...
    {
        try {
            $User = Category::where('KODE', $KODE)->first();
            if (!$User) {
    ...
    public function index()
    {
        try {
            ...
            $categoryT = Category::orderBy('KODE')->get();
            return ApiResponse::json(true, 'Data retrieved successfully',  $categoryT);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to retrieve data', null, 500);
        }
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'NAMA_KATEGORI.required' => 'The NAMA_KATEGORI field is required.',
            'NAMA_KATEGORI.unique' => 'The NAMA_KATEGORI has already been taken.',
        ]);
        ...
        // Check uniqueness for NAMA_KATEGORI case insensitive
        $categoryT = Category::where(DB::raw('LOWER("NAMA_KATEGORI")'), strtolower($request->NAMA_KATEGORI))->first();
        if ($categoryT) {
            return ApiResponse::json(false, ['NAMA_KATEGORI' => ['The NAMA_KATEGORI has already been taken.']], null, 422);
        }
        ...
        try {
            ...
            $new_Category->KODE = $this->getLocalNextId();
            $new_Category->NAMA_KATEGORI = $validatedData['NAMA_KATEGORI'];
            $new_Category->save();
            ...
            $resp_containertype = array(
                'KODE' => $new_Category->KODE,
                'NAMA_KATEGORI' => $new_Category->NAMA_KATEGORI,
            );


            return ApiResponse::json(true, "Data inserted successfully with KODE $new_Category->KODE", $resp_containertype, 201);
        } catch (\Exception $e) {
    ...
    {
        $validator = Validator::make($request->all(), [
            'NAMA_KATEGORI' => 'required|unique:categories,NAMA_KATEGORI,' . $KODE . ',KODE',
        ], [
            'NAMA_KATEGORI.required' => 'The NAMA_KATEGORI field is required.',
        ]);
        ...
        // Check uniqueness for NAMA_KATEGORI case insensitive except for current KODE
        $categoryT = Category::where(DB::raw('LOWER("NAMA_KATEGORI")'), strtolower($request->NAMA_KATEGORI))->where('KODE', '!=', $KODE)->first();
        if ($categoryT) {
            return ApiResponse::json(false, ['NAMA_KATEGORI' => ['The NAMA_KATEGORI has already been taken.']], null, 422);
        }
        ...
        try {
            $validatedData = $validator->validated();
            $categoryT = Category::findOrFail($KODE);
            if ($categoryT->hasRelatedRecords()) {
                return ApiResponse::json(false, 'Cannot update category because it has related records', null, 400);
            }
            $categoryT->NAMA_KATEGORI = $validatedData['NAMA_KATEGORI'];
            $categoryT->save();
            if (!$categoryT) {
            ...
            $resp_containertype = array(
                'KODE' => $categoryT->KODE,
                'NAMA_KATEGORI' => $categoryT->NAMA_KATEGORI,
            );
            return ApiResponse::json(true, 'Category successfully updated', $resp_containertype);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to update Category', null, 500);
        }
    }
    ...
    public function trash()
    {
        ...
        $categoryTs = Category::onlyTrashed()->get();

        return ApiResponse::json(true, 'Trash bin fetched',  $categoryTs);
    }

    public function restore($id)
    {
        $restored = Category::onlyTrashed()->findOrFail($id);
        $restored->restore();
    ...
    {
        $post = Category::onlyTrashed()->findOrFail($id);
    ...
    public function getLocalNextId()
    {
        ...
        $maxId = Category::where('KODE', 'LIKE', 'KTG.%')
            ->withTrashed()
