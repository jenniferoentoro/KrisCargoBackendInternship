...
class OrderTypeController extends Controller
{
    ...
    public function dataTableJson(Request $request)
    {
        ...
        if ($hasSearch) {
            ...
            $attributes = OrderType::first()->getAttributes(); // Get all attribute names
    ...
    public function dataTableAdvJson(Request $request)
    {
        ...
        try {
            ...
            $ordertypes = $allData->slice($skip)->take($limit);
            ...
            $response = [
                ...
                'recordsTotal' => OrderType::count(),
                'recordsFiltered' => $total_data_after_search,
                'data' => $ordertypes->values()->toArray(),
            ];
    ...
    {
        try {
            $User = OrderType::where('KODE', $KODE)->first();
            if (!$User) {
    ...
    public function index()
    {
        try {
            ...
            $orderType = OrderType::orderBy('KODE')->get();
            return ApiResponse::json(true, 'Data retrieved successfully',  $orderType);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to retrieve data', null, 500);
        }
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            //kode unique case insensitive
            'KODE' => 'required|unique:order_types,KODE',
            'NAMA' => 'required|unique:order_types,NAMA',
        ], [
            'NAMA.required' => 'The Order Type field is required.',
            'NAMA.unique' => 'The Order Type has already been taken.',
        ]);
        ...
        $orderType = OrderType::where(DB::raw('LOWER("KODE")'), strtolower($request->KODE))->first();
        if ($orderType) {
            return ApiResponse::json(false, ['KODE' => ['The KODE Order Type has already been taken.']], null, 422);
        }

        // Check uniqueness for NAMA case insensitive
        $orderType = OrderType::where(DB::raw('LOWER("NAMA")'), strtolower($request->NAMA))->first();
        if ($orderType) {
            return ApiResponse::json(false, ['NAMA' => ['The Order Type has already been taken.']], null, 422);
        }
        ...
        try {
            ...
            $new_order_type = new OrderType();
            $new_order_type->KODE = $validatedData['KODE'];
            $new_order_type->NAMA = $validatedData['NAMA'];
            $new_order_type->save();
            ...
            $resp_vendor_type = array(
                'KODE' => $new_order_type->KODE,
                'NAMA' => $new_order_type->NAMA,
            );


            return ApiResponse::json(true, "Data inserted successfully with KODE $new_order_type->KODE", $resp_vendor_type, 201);
        } catch (\Exception $e) {
    ...
    {
        $validator = Validator::make($request->all(), [
            //kode unique case insensitive except for current KODE
            'KODE' => 'required|unique:order_types,KODE,' . $KODE . ',KODE',
            'NAMA' => 'required|unique:order_types,NAMA,' . $KODE . ',KODE',
        ], [
            'NAMA.required' => 'The Order Type field is required.',
        ]);
        ...
        $orderType = OrderType::where(DB::raw('LOWER("KODE")'), strtolower($request->KODE))->where('KODE', '!=', $KODE)->first();
        if ($orderType) {
            return ApiResponse::json(false, ['KODE' => ['The KODE Order Type has already been taken.']], null, 422);
        }

        // Check uniqueness for NAMA case insensitive except for current KODE
        $orderType = OrderType::where(DB::raw('LOWER("NAMA")'), strtolower($request->NAMA))->where('KODE', '!=', $KODE)->first();
        if ($orderType) {
            return ApiResponse::json(false, ['NAMA' => ['The Order Type has already been taken.']], null, 422);
        }
        ...
        try {
            $validatedData = $validator->validated();
            $orderTypee = OrderType::findOrFail($KODE);
            $orderTypee->KODE = $validatedData['KODE'];
            $orderTypee->NAMA = $validatedData['NAMA'];
            $orderTypee->save();
            if (!$orderTypee) {
                return ApiResponse::json(false, 'Failed to update Order Type', null, 500);
            }
            $resp_vendortypee = array(
                'KODE' => $orderTypee->KODE,
                'NAMA' => $orderTypee->NAMA,
            );
            return ApiResponse::json(true, 'Order Type successfully updated', $resp_vendortypee);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to update Order Type', null, 500);
        }
    }
    ...
    {
        try {
            $negara = OrderType::findOrFail($KODE);
            $negara->delete();
            return ApiResponse::json(true, 'Order Type successfully deleted', ['KODE' => $KODE]);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to delete Order Type', null, 500);
        }
    }
    ...
    public function trash()
    {
        ...
        $negaras = OrderType::onlyTrashed()->get();
    ...
    {
        $restored = OrderType::onlyTrashed()->findOrFail($id);
        $restored->restore();
    ...
    {
        $post = OrderType::onlyTrashed()->findOrFail($id);
    ...
    {
        // $tableName = 'order_types'; // Replace with your table name
        ...
        $maxKODE = OrderType::withTrashed()->max('KODE');
        //get next KODE
        $nextKODE = $maxKODE + 1;
