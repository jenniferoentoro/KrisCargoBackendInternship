...
class ServiceController extends Controller
{
    ...
    public function dataTableJson(Request $request)
    {
        ...
        if ($hasSearch) {
            ...
            $attributes = Service::first()->getAttributes(); // Get all attribute names
        ...
        $services = $allData->slice($skip)->take($limit);
    ...
    {
        try {
            $serviceT = Service::where('KODE', $KODE)->first();
            //convert to array
            $serviceT = $serviceT->toArray();
            return ApiResponse::json(true, 'Service retrieved successfully',  $serviceT);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to retrieve Service', null, 500);
        }
    }
    ...
    {
        try {
            //get all Service order by KODE
            $serviceT = Service::orderBy('KODE')->get();
            return ApiResponse::json(true, 'Data retrieved successfully',  $serviceT);
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        ...
        $serviceT = Service::where(DB::raw('LOWER("KODE")'), strtolower($request->KODE))->first();
        if ($serviceT) {
            return ApiResponse::json(false, ['KODE' => ['The KODE Service has already been taken.']], null, 422);
        }

        // Check uniqueness for NAMA case insensitive
        $serviceT = Service::where(DB::raw('LOWER("NAMA")'), strtolower($request->NAMA))->first();
        if ($serviceT) {
        ...
        try {
            ...
            $new_service->KODE = $validatedData['KODE'];
            $new_service->NAMA = $validatedData['NAMA'];
            $new_service->save();
            ...
            $resp_containertype = array(
                'KODE' => $new_service->KODE,
                'NAMA' => $new_service->NAMA,
            );


            return ApiResponse::json(true, "Data inserted successfully with KODE $new_service->KODE", $resp_containertype, 201);
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $KODE)
    {
        $validator = Validator::make($request->all(), [
            //KODE unique
            'KODE' => 'required|unique:services,KODE,' . $KODE . ',KODE',
            'NAMA' => 'required|unique:services,NAMA,' . $KODE . ',KODE',
        ], [
            'NAMA.required' => 'The NAMA field is required.',
        ]);
        ...
        $serviceT = Service::where(DB::raw('LOWER("KODE")'), strtolower($request->KODE))->where('KODE', '!=', $KODE)->first();
        if ($serviceT) {
            return ApiResponse::json(false, ['KODE' => ['The KODE Service has already been taken.']], null, 422);
        }

        // Check uniqueness for NAMA case insensitive except for current KODE
        $serviceT = Service::where(DB::raw('LOWER("NAMA")'), strtolower($request->NAMA))->where('KODE', '!=', $KODE)->first();
        if ($serviceT) {
            return ApiResponse::json(false, ['NAMA' => ['The NAMA has already been taken.']], null, 422);
        }
        ...
        try {
            $validatedData = $validator->validated();
            $serviceT = Service::findOrFail($KODE);
            $serviceT->KODE = $validatedData['KODE'];
            $serviceT->NAMA = $validatedData['NAMA'];
            $serviceT->save();
            if (!$serviceT) {
                return ApiResponse::json(false, 'Failed to update Service', null, 500);
            }
            $resp_containertype = array(
                'KODE' => $serviceT->KODE,
                'NAMA' => $serviceT->NAMA,
            );
            return ApiResponse::json(true, 'Service successfully updated', $resp_containertype);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to update Service', null, 500);
        }
    }
    ...
    public function destroy($KODE)
    {
        try {
            $serviceT = Service::findOrFail($KODE);
            $serviceT->delete();
            return ApiResponse::json(true, 'Service successfully deleted', ['KODE' => $KODE]);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to delete Service', null, 500);
        }
    }
    ...
    public function trash()
    {
        ...
        $serviceTs = Service::onlyTrashed()->get();

        return ApiResponse::json(true, 'Trash bin fetched',  $serviceTs);
    }

    public function restore($id)
    {
        $restored = Service::onlyTrashed()->findOrFail($id);
        $restored->restore();
    ...
    {
        $post = Service::onlyTrashed()->findOrFail($id);
    ...
    {
        // $tableName = 'services'; // Replace with your table name
        ...
        $maxKODE = Service::withTrashed()->max('KODE');
        //get next KODE
        $nextKODE = $maxKODE + 1;
