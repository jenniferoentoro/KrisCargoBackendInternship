...
use App\Models\Size;
use App\Models\ThcLoloPortDischarge;
...
class ThcLoloPortDischargeController extends Controller
{
    public function dataTableJson(Request $request)
    {
        ...
        $query = ThcLoloPortDischarge::query();
        ...
        if ($hasSearch) {
            ...
            $attributes = ThcLoloPortDischarge::first()->getAttributes(); // Get all attribute names
        ...
        $response = [
            ...
            'recordsTotal' => ThcLoloPortDischarge::count(),
    ...
    {
        try {
            $thclolo = ThcLoloPortDischarge::where('KODE', $KODE)->first();
            //convert to array
            $thclolo = $thclolo->toArray();
            return ApiResponse::json(true, 'THC LOLO Port of Discharge retrieved successfully',  $thclolo);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to retrieve THC LOLO Port of Discharge', null, 500);
        }
    }
    ...
    public function index()
    {
        try {
            //order by KODE
            $thclolos = ThcLoloPortDischarge::orderBy('KODE')->get();
            foreach ($thclolos as $thclolo) {
                $thclolo->NAMA_VENDOR = Vendor::where('KODE', $thclolo->KODE_VENDOR)->first()->NAMA;
                $thclolo->NAMA_PELABUHAN = Harbor::where('KODE', $thclolo->KODE_PELABUHAN)->first()->NAMA_PELABUHAN;
                $thclolo->NAMA_UK_KONTAINER = Size::where('KODE', $thclolo->KODE_UK_KONTAINER)->first()->NAMA;
    ...
    public function storeWeb(Request $request)
    {
        ...
        // $existingData = ThcLoloPortDischarge::where(DB::raw('LOWER("KAPAL")'), strtolower($request->KAPAL))->first();
        // if ($existingData) {
        ...
        try {
            ...
            $new_ThcLoloPortDischarge = new ThcLoloPortDischarge();
            $new_ThcLoloPortDischarge->KODE = $this->getLocalNextId();
            $new_ThcLoloPortDischarge->KODE_VENDOR = $request->KODE_VENDOR;
            $new_ThcLoloPortDischarge->KODE_PELABUHAN = $request->KODE_PELABUHAN;
            $new_ThcLoloPortDischarge->KODE_UK_KONTAINER = $request->KODE_UK_KONTAINER;
            $new_ThcLoloPortDischarge->KODE_JENIS_KONTAINER = $request->KODE_JENIS_KONTAINER;
            $new_ThcLoloPortDischarge->THC = $request->THC;
            $new_ThcLoloPortDischarge->LOLO_LUAR = $request->LOLO_LUAR;
            $new_ThcLoloPortDischarge->LOLO_DALAM = $request->LOLO_DALAM;
            $new_ThcLoloPortDischarge->TGL_MULAI_BERLAKU = $request->TGL_MULAI_BERLAKU;
            $new_ThcLoloPortDischarge->TGL_AKHIR_BERLAKU = $request->TGL_AKHIR_BERLAKU;


            $new_ThcLoloPortDischarge->save();
            if (!$new_ThcLoloPortDischarge) {
                return ApiResponse::json(false, 'Failed to insert data', null, 500);
            }
            $id = $new_ThcLoloPortDischarge->KODE;

            $thclolo = ThcLoloPortDischarge::where('KODE', $id)->first();
    ...
    public function update(Request $request, $KODE)
    {
        ...
        // $existingData = ThcLoloPortDischarge::where(DB::raw('LOWER("KAPAL")'), strtolower($request->KAPAL))->where('KODE', '!=', $KODE)->first();
        // if ($existingData) {
        //     return ApiResponse::json(false, ['KAPAL' => ['The KAPAL has already been taken.']], null, 422);
        // }
        try {

            $thclolo = ThcLoloPortDischarge::findOrFail($KODE);
            ...
            return ApiResponse::json(true, 'THC LOLO Port of Discharge successfully updated', $resp_ThcLoloPortDischarge);
        } catch (\Exception $e) {
            return ApiResponse::json(false, $e, null, 500);
        }
    }
    ...
    public function destroy($KODE)
    {
        try {
            $thclolo = ThcLoloPortDischarge::findOrFail($KODE);
            $thclolo->delete();
            return ApiResponse::json(true, 'THC LOLO Port of Discharge successfully deleted', ['KODE' => $KODE]);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to delete THC LOLO Port of Discharge', null, 500);
        }
    }

    public function trash()
    {
        $thclolos = ThcLoloPortDischarge::onlyTrashed()->get();
    ...
    {
        $restored = ThcLoloPortDischarge::onlyTrashed()->findOrFail($id);
        $restored->restore();
    ...
    public function getLocalNextId()
    {
        ...
        $maxId = ThcLoloPortDischarge::where('KODE', 'LIKE', 'TLD.%')
            ->withTrashed()
