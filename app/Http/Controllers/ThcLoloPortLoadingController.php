...
use App\Models\Size;
use App\Models\ThcLoloPortLoading;
use App\Models\Vendor;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Validator;

class ThcLoloPortLoadingController extends Controller
{
    public function dataTableJson(Request $request)
    {
        ...
        $query = ThcLoloPortLoading::query();
        ...
        if ($hasSearch) {
            ...
            $attributes = ThcLoloPortLoading::first()->getAttributes(); // Get all attribute names
        ...
        $response = [
            ...
            'recordsTotal' => ThcLoloPortLoading::count(),
    ...
    {
        try {
            $thclolo = ThcLoloPortLoading::where('KODE', $KODE)->first();
            //convert to array
            $thclolo = $thclolo->toArray();
            return ApiResponse::json(true, 'THC LOLO Port of Loading retrieved successfully',  $thclolo);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to retrieve THC LOLO Port of Loading', null, 500);
        }
    }
    ...
    public function index()
    {
        try {
            //order by KODE
            $thclolos = ThcLoloPortLoading::orderBy('KODE')->get();
            foreach ($thclolos as $thclolo) {
                $thclolo->NAMA_VENDOR = Vendor::where('KODE', $thclolo->KODE_VENDOR)->first()->NAMA;
                $thclolo->NAMA_PELABUHAN = Harbor::where('KODE', $thclolo->KODE_PELABUHAN)->first()->NAMA_PELABUHAN;
                $thclolo->NAMA_UK_KONTAINER = Size::where('KODE', $thclolo->KODE_UK_KONTAINER)->first()->NAMA;
    ...
    public function storeWeb(Request $request)
    {
        ...
        // $existingData = ThcLoloPortLoading::where(DB::raw('LOWER("KAPAL")'), strtolower($request->KAPAL))->first();
        // if ($existingData) {
        ...
        try {
            ...
            $new_ThcLoloPortLoading = new ThcLoloPortLoading();
            $new_ThcLoloPortLoading->KODE = $this->getLocalNextId();
            $new_ThcLoloPortLoading->KODE_VENDOR = $request->KODE_VENDOR;
            $new_ThcLoloPortLoading->KODE_PELABUHAN = $request->KODE_PELABUHAN;
            $new_ThcLoloPortLoading->KODE_UK_KONTAINER = $request->KODE_UK_KONTAINER;
            $new_ThcLoloPortLoading->KODE_JENIS_KONTAINER = $request->KODE_JENIS_KONTAINER;
            $new_ThcLoloPortLoading->THC = $request->THC;
            $new_ThcLoloPortLoading->LOLO_LUAR = $request->LOLO_LUAR;
            $new_ThcLoloPortLoading->LOLO_DALAM = $request->LOLO_DALAM;
            $new_ThcLoloPortLoading->TGL_MULAI_BERLAKU = $request->TGL_MULAI_BERLAKU;
            $new_ThcLoloPortLoading->TGL_AKHIR_BERLAKU = $request->TGL_AKHIR_BERLAKU;


            $new_ThcLoloPortLoading->save();
            if (!$new_ThcLoloPortLoading) {
                return ApiResponse::json(false, 'Failed to insert data', null, 500);
            }
            $id = $new_ThcLoloPortLoading->KODE;

            $thclolo = ThcLoloPortLoading::where('KODE', $id)->first();
            $resp_ThcLoloPortLoading = array(
                'KODE' => $thclolo->KODE,
            ...
            return ApiResponse::json(true, "Data inserted successfully with KODE $thclolo->KODE", $resp_ThcLoloPortLoading, 201);
        } catch (\Exception $e) {
            return ApiResponse::json(false, $e, null, 500);
        }
    }
    ...
    public function update(Request $request, $KODE)
    {
        ...
        // $existingData = ThcLoloPortLoading::where(DB::raw('LOWER("KAPAL")'), strtolower($request->KAPAL))->where('KODE', '!=', $KODE)->first();
        // if ($existingData) {
        //     return ApiResponse::json(false, ['KAPAL' => ['The KAPAL has already been taken.']], null, 422);
        // }
        try {

            $thclolo = ThcLoloPortLoading::findOrFail($KODE);
            ...
            return ApiResponse::json(true, 'THC LOLO Port of Loading successfully updated', $resp_ThcLoloPortLoading);
        } catch (\Exception $e) {
            return ApiResponse::json(false, $e, null, 500);
        }
    }
    ...
    public function destroy($KODE)
    {
        try {
            $thclolo = ThcLoloPortLoading::findOrFail($KODE);
            $thclolo->delete();
            return ApiResponse::json(true, 'THC LOLO Port of Loading successfully deleted', ['KODE' => $KODE]);
        } catch (\Exception $e) {
            return ApiResponse::json(false, 'Failed to delete THC LOLO Port of Loading', null, 500);
        }
    }

    public function trash()
    {
        $thclolos = ThcLoloPortLoading::onlyTrashed()->get();

        $resp_ThcLoloPortLoadings = [];

        foreach ($thclolos as $thclolo) {
            $resp_ThcLoloPortLoading = [
                'KODE' => $thclolo->KODE,
                'NAMA_VENDOR' => Vendor::where('KODE', $thclolo->KODE_VENDOR)->first()->NAMA,
                'NAMA_PELABUHAN' => Harbor::where('KODE', $thclolo->KODE_PELABUHAN)->first()->NAMA_PELABUHAN,
                'NAMA_UK_KONTAINER' => Size::where('KODE', $thclolo->KODE_UK_KONTAINER)->first()->NAMA,
            ...
            $resp_ThcLoloPortLoadings[] = $resp_ThcLoloPortLoading;
        }

        return ApiResponse::json(true, 'Trash bin fetched', $resp_ThcLoloPortLoadings);
    }
    ...
    {
        $restored = ThcLoloPortLoading::onlyTrashed()->findOrFail($id);
        $restored->restore();
    ...
    public function getLocalNextId()
    {
        ...
        $maxId = ThcLoloPortLoading::where('KODE', 'LIKE', 'TLL.%')
            ->withTrashed()
